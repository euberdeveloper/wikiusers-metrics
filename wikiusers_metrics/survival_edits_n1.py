...
def retriever(month_activity: dict) -> float:
    try:
        return month_activity['events']['per_namespace']['n1']['total']
    except:
        return 0


if __name__ == '__main__':
    metricher = SurvivalMetricher(name='n1_edits_over_survived_months', dropoff_month_threshold=24, lang='it', discriminator=active_discriminator, extractor=retriever)
    metricher.compute()
    metricher.save_json()
